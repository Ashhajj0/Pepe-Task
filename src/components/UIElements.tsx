...
export const NavItem = memo(({ icon, active, label, onClick }: { icon: ReactNode, active?: boolean, label: string, onClick: () => void }) => {
  return (
    <button 
      onClick={onClick} 
      className={`flex flex-col items-center justify-center gap-1 transition-all outline-none relative px-1 py-3 rounded-[24px] ${active ? 'text-slate-900' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50/50'}`}
    >
      {active && (
        <motion.div 
          layoutId="activeNav"
